...
//                      vector<uint32_t>& tour, vector<uint16_t>& whichSlot, 
//                      uint32_t minLink, uint32_t& maxLink, 
//                      const chrono::steady_clock::time_point startTime, const chrono::milliseconds timeLimit) {
...
//             // moved here from outer loop because of timelimit issues
 ...
 * TODO: update maxLink in this function so we do not need to update it in each iteration over the complete tour
 */
...
int main() {
    ...
    while (!timeOver(startTime, timeLimit)) {
        ...
        // fastThreeOpt(distanceMatrix, nbhd, trialTour, whichSlot, minLink, maxLink, iterationStartTime, timePerIteration);
        // improved = improved || setBestTour(distanceMatrix, bestTour, bestCost, trialTour);
